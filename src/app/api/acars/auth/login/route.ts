...
/**
 * Autenticacion dedicada para ACARS cliente - SOLO NEON.
 * Endpoint: POST /api/acars/auth/login
 * 
 * NO Supabase Auth. NO fallback Supabase.
 * Solo usa tablas Neon: app_users, app_user_credentials, pilot_profiles.
 */
export async function POST(request: Request) {
  try {
    ...
    // SOLO login con sistema Neon (custom auth)
    ...
    if (!pilot || !pilot.callsign) {
      return NextResponse.json(
        { ok: false, code: "PILOT_NOT_FOUND", error: "Perfil de piloto no encontrado en Neon. Contacte soporte." },
        { status: 403 }
      );
    }
