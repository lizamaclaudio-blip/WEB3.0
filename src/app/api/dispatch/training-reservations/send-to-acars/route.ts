...
function errorResponse(error: unknown) {
  ...
  return NextResponse.json(
    { ok: false, error: code, message: messageByCode[code] ?? "No se pudo preparar el despacho para ACARS." },
    { status },
  );
}
...
export async function POST(request: Request) {
  try {
    ...
    const acarsPayload = await prepareTrainingReservationForAcars(user, {
      reservationId: String(body.reservationId ?? ""),
      dispatchToken: String(body.dispatchToken ?? ""),
    });
